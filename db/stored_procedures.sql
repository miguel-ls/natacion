...
BEGIN
    INSERT INTO cursos (nombre, descripcion, precio_base) VALUES (p_nombre, p_descripcion, p_precio_base);
END$$

-- Obtener todos los cursos
CREATE PROCEDURE `sp_get_all_cursos`()
BEGIN
    SELECT id_curso, nombre, descripcion, precio_base FROM cursos;
END$$
...
BEGIN
    SELECT id_curso, nombre, descripcion, precio_base FROM cursos WHERE id_curso = p_id_curso;
END$$
...
BEGIN
    UPDATE cursos
    SET nombre = p_nombre, descripcion = p_descripcion, precio_base = p_precio_base
...
BEGIN
    -- Se debe tener cuidado con las dependencias (matriculas).
    -- Lo ideal sería anular matrículas antes de borrar.
...
BEGIN
    ...
    -- Obtener capacidad del carril para ese horario
    ...
    -- Contar alumnos matriculados y activos en ese horario
...
BEGIN
    ...
    WHILE v_current_date <= p_fecha_fin DO
        -- DAYOFWEEK() retorna 1 para Domingo, 2 para Lunes, etc.
