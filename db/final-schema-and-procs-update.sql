...
-- -----------------------------------------------------------------
-- 1. Creación de la tabla para agrupar matrículas múltiples
-- -----------------------------------------------------------------
...
-- -----------------------------------------------------------------
-- 2. Modificación de la tabla de matrículas para añadir la referencia al grupo
--    Se comprueba si la columna ya existe para evitar errores.
-- -----------------------------------------------------------------
SET @db = 'natacion_bd';
SET @table = 'matriculas';
SET @column = 'id_grupo_matricula';
SET @sql = CONCAT('SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = "', @db, '" AND table_name = "', @table, '" AND column_name = "', @column, '" INTO @col_exists');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @sql_alter = IF(@col_exists = 0,
    'ALTER TABLE `matriculas` ADD COLUMN `id_grupo_matricula` INT NULL DEFAULT NULL AFTER `observaciones`, ADD INDEX `fk_matriculas_grupos_idx` (`id_grupo_matricula` ASC), ADD CONSTRAINT `fk_matriculas_grupos` FOREIGN KEY (`id_grupo_matricula`) REFERENCES `matricula_grupos` (`id_grupo_matricula`) ON DELETE SET NULL ON UPDATE CASCADE;',
    'SELECT "La columna id_grupo_matricula ya existe." as msg;'
);
...
-- -----------------------------------------------------------------
-- 3. Procedimiento para crear una matrícula (versión final)
-- -----------------------------------------------------------------
DELIMITER $$
...
-- -----------------------------------------------------------------
-- 4. Procedimiento para obtener todos los profesores (versión final)
-- -----------------------------------------------------------------
DELIMITER $$
...
-- -----------------------------------------------------------------
-- 5. Procedimiento para obtener grupos de matrícula (versión final)
-- -----------------------------------------------------------------
DELIMITER $$
...
-- -----------------------------------------------------------------
-- 6. Procedimiento para obtener detalles de un grupo de matrícula
-- -----------------------------------------------------------------
DELIMITER $$
...
-- -----------------------------------------------------------------
-- 7. Procedimiento para encontrar carriles libres
-- -----------------------------------------------------------------
DELIMITER $$
...
-- -----------------------------------------------------------------
-- 8. Procedimiento para obtener clases para el calendario
-- -----------------------------------------------------------------
DELIMITER $$
